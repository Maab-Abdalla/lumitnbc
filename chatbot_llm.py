"""
LumiTNBC chatbot: lightweight RAG (retrieval) + OpenAI answer.
...
Flow:
  ...
  3. generate_reply(question, history) injects those chunks into a scoped system
     prompt and calls the OpenAI API with MODERATE medical guardrails:
...
Degrades gracefully: if no OPENAI_API_KEY is set, or the package/network is
...
# Load .env if present so OPENAI_API_KEY is available even when this module is
# used outside the Flask app (e.g. scripts, tests). Safe to call repeatedly.
try:
